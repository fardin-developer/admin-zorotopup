...
const items: MenuProps['items'] = [
  ...
  getItem('Dashboards', 'dashboards', <PieChartOutlined />, [
    getItem(
      <Link to={PATH_DASHBOARD.ecommerce}>Sales & Analaytics</Link>,
      'ecommerce',
      null
    )
  ]),
...
const rootSubmenuKeys: string[] = ['games']; // Include games dropdown
...
const SideNav = ({ ...others }: SideNavProps) => {
  ...
  useEffect(() => {
    ...
    if (pathname === '/') {
      ...
      setOpenKeys(['games']);
      if (pathname.includes('game') && !pathname.includes('create-packages')) {
        setCurrent('game');
      ...
      setCurrent(paths[paths.length - 1] || 'dashboard');
      setOpenKeys([]);
    }
  }, [pathname]);
